...
def MainFitEllipseModel(data, mask_cr=None, geometry=None, make_plots=False, plot_plots=False, sma_normfactor=1, sma_rescale=1, final=False, image_path=None, method='v1'):
    ...
    if method == 'v1':
        ...
        nclip = nclip_sm
        while nclip <= 3:
            isolist = ellipse.fit_image(nclip=nclip, fflag=0.6, step=0.1, fix_center=True, fix_pa=True, inside_non_fixed=True)
            if len(isolist)!=0:
                break
            else: 
                nclip += 1
...
def build_elliptical_model_with_subpixels(
    ...
    Important: does not take masked arrays, put Nans for values to be masked instead
    ...
    for s in sma_values:
        ...
        med = np.nanmedian(clipped) if clipped.size > 0 else 0
...
def fitApertureModel(data, mask_cr=None, make_plots=False, plot_plots=False, geometry=None, sma_normfactor=1, final=False, image_path=None, sclipmaxiters=5):
    ...
    # The elliptical model function prefers to take data where the masked items are replaced with nans, so:
    nan_data = masked_data.filled(np.nan)
    model_basic, intensities = build_elliptical_model_with_subpixels(nan_data,
        geometry.x0,
        geometry.y0,
        geometry.eps,
        geometry.pa,
