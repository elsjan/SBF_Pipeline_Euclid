...
for o_id in dwarfs_cat['Name']:
    ...
    flag = table_flagged.loc[table_flagged['OBJECT_ID'] == name, 'flag2'].iloc[0]
    # print(flag)
    if flag != 1:
        cutout_size = round(dwarfs_cat.loc[dwarfs_cat['Name']==o_id,'Re'].iloc[0]*60)
        ...
        try:
            ...
            fieldcutoutI = Cutout2D(full_dataI, position, (cutout_size*6*fc, cutout_size*6*fc), wcs=wcsI, copy=True)
            ...
            hdu_fieldcutoutI.data[cutout_size*3*fc-cutout_size*3//2:cutout_size*3*fc+cutout_size*3//2, cutout_size*3*fc-cutout_size*3//2:cutout_size*3*fc+cutout_size*3//2] = np.nan
            # cutout_size*3//2:cutout_size*9//2, cutout_size*3//2:cutout_size*9//2]
            try:
            ...
            fieldcutoutH = Cutout2D(full_dataH, position, (cutout_size*2*fc, cutout_size*2*fc), wcs=wcsH, copy=True)
            ...
            hdu_fieldcutoutH.data[cutout_size*fc-cutout_size//2:cutout_size*fc+cutout_size//2, cutout_size*fc-cutout_size//2:cutout_size*fc+cutout_size//2 ] = np.nan
            # cutout_size//2:cutout_size*3//2, cutout_size//2:cutout_size*3//2]
            try:
