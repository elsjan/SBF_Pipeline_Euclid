        ...
        for kfit_i_iter in kfit_i:
            for kfit_f_iter in kfit_f:
                ...
                if len(psf.shape) == 2:
                    ...
                    noises = []
                    for i in range(len(expected_ps[:,0])):
                        sbfi, noisei = fitSbfComponents(image_ps, expected_ps[i,:], kfit_i_iter, kfit_f_iter, 
                                        image_path=None, image_title=image_title, 
                                        make_plots=False,plot_plots=False, plotyrange=plotyrange)
                        sbfs.append(sbfi)
                    sbfs = np.array(sbfs)
                    ...
                    # sbf = sbfs[idx]
                    # noise = noises[idx]
                    expected_ps_step = expected_ps[idx,:]
                    sbf, noise = fitSbfComponents(image_ps, expected_ps_step, kfit_i_iter, kfit_f_iter, 
