...
def unmaskMaxArea(objects, segmap, max_area):
    """
    Remove sources whose segmentation area exceeds max_area pixels.
    These sources are removed from both the segmentation map and the
    objects array.
    """

    # Compute area (number of pixels) per segmentation label
    areas = np.bincount(segmap.ravel())

    # Labels to remove: area > max_area, excluding background (label 0)
    labels_to_remove = np.where(areas > max_area)[0]
    labels_to_remove = labels_to_remove[labels_to_remove != 0]

    if labels_to_remove.size == 0:
        return objects, segmap

    # Remove labels from segmentation map
    for label in labels_to_remove:
        segmap[segmap == label] = 0

    # SEP convention: object index = label - 1
    obj_indices = labels_to_remove - 1

    # Guard against out-of-bounds (rare but safe)
    obj_indices = obj_indices[obj_indices < len(objects)]

    objects = np.delete(objects, obj_indices, axis=0)
...
def maskBackgroundSources(data, mask_cr=None, make_plots=False, plot_plots=False, detect_thresh=3, minarea=7, maxarea=None, r=2.5, image_path=None, 
    ...
    try:
    # Detect sources
        objects, segmap = sep.extract(data.astype(np.float64), thresh=detect_thresh * bkg.globalrms,
